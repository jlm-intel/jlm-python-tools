"""
General-purpose file utility methods.
"""
...
def copy_file(source_path, target_path, move_file=False):
    """
    Copy a file from a source path to a target location, optionally deleting the source file upon successful copy.

    Returns: A result code from FileUtilitiesConstants
    """
...
def copy_newer_file(file_one_path, file_two_path, move_file=False, sync_mode=False, dry_run=False, verbose=True):
    """
    Only copies a file if one version is newer than the other. Optionally removes the source file after successful copy (move_file).
    Also, optionally performs reverse copy (target to source) if sync_mode is enabled and the second file is newer than the first.
    Optional dry-run mode to test the sync logic without actually copying/moving any files. This method is for applications where
    you want to keep two directories in sync (both have the latest versions of their files).
...
def save_json_file(file_path, data_blob):
    """
    Saves a given blob of data to a JSON file.
...
def load_json_file(file_path):
    """
    Loads a JSON-formatted file and returns its contents as a JSON blob.

    Returns: None on failure, otherwise returns the contents as JSON.
    """
    f = None
    blob = None
...
def verify_directory(directory_path, make_if_missing=True, verbose=False):
    """
    Verifies the existence of a given directory. Optionally creates the directory if it doesn't currently exist.
    """
    success = False
...
def is_long_path(cur_path):
    """
    Determines whether a given path is a Windows-style long path/UNC.
    """
...
def make_long_path(cur_path):
    """
    Converts a given file path to a Windows-style long path/UNC.
    """
