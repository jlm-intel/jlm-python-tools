"""
Searches a given directory for a list of full or partial filenames, copying matching files to a target directory. For example,
you could search a nested directory of specific audio samples and consolidate them all into a target directory location.
Normally the file list is a simple text file with one filename on each line. You can optionally pass the output from
the open-source cubase-project-plugins tool (https://github.com/fgimian/cubase-project-plugins) to use that as the basis
to search from. (The script will extract the names of 32-bit or 64-bit plugins referenced in the file and search the
source directory for matching filenames. I added this feature when I was trimming out unused plugins from my DAW PC.)
"""
...
from file_utilities import (  # for my file methods
    FileUtilitiesConstants,
    copy_file,
    load_text_file,
    verify_directory,
)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Searches a given source directory for a given list of full or partial filenames, and copies any matches to a given target directory."
    )
    parser.add_argument(
        "--file_list",
        type=str,
        required=True,
        help="File containing full or partial filenames to search for in source directory.",
    )
    parser.add_argument(
        "--source",
        type=str,
        required=True,
        help="Directory to search for files. (Can be nested.)",
    )
    parser.add_argument(
        "--target",
        type=str,
        required=True,
        help="Directory to copy found files to. (Single flat directory.)",
    )
    parser.add_argument(
        "--plugins32",
        action="store_true",
        help="Indicates that file_list is a cubase-project-plugins report, and you want to extract 32-bit plugin names from it.",
    )
    parser.add_argument(
        "--plugins64",
        action="store_true",
        help="Indicates that file_list is a cubase-project-plugins report, and you want to extract 64-bit plugin names from it.",
    )
    parser.add_argument(
        "--keep_tree",
        action="store_true",
        help="Reproduces the directory structure fron source instead of copying everything to a flat directory.",
    )
...
def extract_plugin_names(lines, get32bit, get64bit):
    ...
    if (not get32bit) and (not get64bit):
