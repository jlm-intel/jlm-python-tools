"""
Batch rename script to recursively search for files of a given filename and rename them to a target filename, overwriting existing files with the target name.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Recursively searches for files of a given filename and renames them to a target filename, overwriting existing files with the target name."
    )
    parser.add_argument("dir", type=str, help="Directory to search for source files.")
    parser.add_argument("--source", type=str, default="new_reference.json", required=False, help="Source filename to search for in directory.")
    parser.add_argument("--target", type=str, default="golden_reference.json", required=False, help="Target filename to copy found source files to.")
...
if __name__ == "__main__":
    ...
    # if none found, end
    if len(filelist) == 0:
        print("ERROR: No files found matching " + source_file + " under " + search_dir)
        quit()

    for curfile in filelist:
        # if found, build new path to target file
        new_target = os.path.join(os.path.dirname(curfile), target_file)
        ...
        try:
            ...
            print("OS error occurred: " + new_target + " (" + str(error) + ")")
            errors += 1
