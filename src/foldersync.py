"""
Syncs local directories to a remote storage location, optionally pruning out obsolete backup files.

Different applications might be:
- Syncing local directories to a backup drive, with optional pruning of deleted files to save space on the backup drive.
- If you work on multiple computers, you can use this to sync local files to a shared cloud folder.
"""
...
from file_utilities import FileUtilitiesConstants, copy_newer_file, load_json_file, make_long_path, save_json_file, verify_directory  # for my file methods
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Syncs local directories to a remote storage location, optionally pruning out obsolete backup files.")
...
def sync_directories(settings_path, dry_run, debug_limit, prune, verbose):
    ...
    for cur_volume in settings["remote_volumes"]:
        ...
        volume_stats[cur_volume] = {"free_before": stat.free, "free_after": 0}
...
def create_settings(settings_path):
    """
    Creates a template settings file which you must populate with your desired settings. The settings file is a JSON file with the following format:
       "remote_volumes" - list of backup drives, so we can collect before/after usage information
       "profiles" - named pairs of local and remote directories to sync (videos, documents, music, etc), same format as multisync.py's profiles.
    The idea is to provide a flexible way to define multiple backup sets with different source and destination directories. You can set up your
    own profiles based on your needs, with full control over the source and target directories.

    Here's a small example of a populated settings file that backs up the local Documents, Downloads, and Music folders to a remote backup drive.
    Note that the names of the profiles are arbitrary (they don't have to correspond to actual directory names). Also, Windows files paths must be
    escaped with double backslashes (\\) or use raw strings (r"string") to avoid issues with single backslashes:
    {
        "remote_volumes": [
            "\\\\BOOEYCLOUD\\home\\Drive\\Backup\\ANTEC-P280\\C\\Users\\joshm"
        ],
        "profiles": {
            "Documents": {
                "local_dir": "C:\\Users\\joshm\\Documents",
                "remote_dir": "\\\\BOOEYCLOUD\\home\\Drive\\Backup\\ANTEC-P280\\C\\Users\\joshm\\Documents"
            },
            "Downloads": {
                "local_dir": "C:\\Users\\joshm\\Downloads",
                "remote_dir": "\\\\BOOEYCLOUD\\home\\Drive\\Backup\\ANTEC-P280\\C\\Users\\joshm\\Downloads"
            },
            "Music": {
                "local_dir": "C:\\Users\\joshm\\Music",
                "remote_dir": "\\\\BOOEYCLOUD\\home\\Drive\\Backup\\ANTEC-P280\\C\\Users\\joshm\\Music"
            }
        }
    }
    """
    ...
    blob = {
        "remote_volumes": ["(backup_path_1)", "(backup_path_2)"],
        "profiles": {
            "Documents": {"local_dir": "local_full_path", "remote_dir": "remote_full_path"},
            "Downloads": {"local_dir": "local_full_path", "remote_dir": "remote_full_path"},
            "Music": {"local_dir": "local_full_path", "remote_dir": "remote_full_path"},
            "Pictures": {"local_dir": "local_full_path", "remote_dir": "remote_full_path"},
            "Videos": {"local_dir": "local_full_path", "remote_dir": "remote_full_path"},
        },
    }
